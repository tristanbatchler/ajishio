...
_screen: pg.Surface = _engine._screen
...
def draw_circle(x: float, y: float, radius: float, color: Color | None = None) -> None:
    pg.draw.circle(_screen, _draw_color if color is None else color, (x, y), radius)

def draw_rectangle(x: float, y: float, room_width: float, room_height: float, outline: bool = False, color: Color | None = None) -> None:
    pg.draw.rect(_screen, _draw_color if color is None else color, (x, y, room_width, room_height), int(outline))

def draw_line(x1: float, y1: float, x2: float, y2: float, color: Color | None = None) -> None:
    pg.draw.line(_screen, _draw_color if color is None else color, (x1, y1), (x2, y2))
