...
class PhysicsObject(aj.GameObject):
    ...
    def step(self) -> None:
        ...
        self.y_velocity = aj.clamp(
            self.y_velocity + self.gravity, -self.max_fall_speed, self.max_fall_speed
        )
...
class Player(PhysicsObject):
    ...
    def step(self) -> None:
        ...
        if (
            not self.place_meeting(self.x, self.y + 1, Floor)
            and self.y_velocity < 0
            and not aj.keyboard_check(aj.vk_space)
        ):
            self.y_velocity /= 1.5
    ...
    def move_through_doorway(self, doorway: Doorway) -> None:
        ...
        to_doorway = aj.instance_find(doorway.to_doorway_iid)
        if to_doorway is None:
            raise ValueError(
                f"Doorway with iid {doorway.to_doorway_iid} not found in room {doorway.to_room}"
            )
...
class Camera(aj.GameObject):
    ...
    @override
    def step(self) -> None:
        player = aj.instance_find(Player)
        if player is None:
            return
...
class Enemy(PhysicsObject):
    ...
    def step(self) -> None:
        ...
        if not self.place_meeting(
            self.x + self.sprite_width * aj.sign(self.x_velocity) + self.x_velocity,
            self.y + 1,
            Floor,
        ):
            self.x_velocity *= -1
...
class Coin(aj.GameObject):
    ...
    def step(self) -> None:
        super().step()
        player_hit = self.place_meeting(self.x, self.y, Player)
