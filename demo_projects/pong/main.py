...
class Wall(aj.GameObject):
    ...
    def draw(self) -> None:
        aj.draw_rectangle(
            self.x, self.y, self.x + self.width, self.y + self.height, outline=True
        )
...
class Ball(aj.GameObject):
    ...
    def draw(self) -> None:
        aj.draw_circle(
            ...
            self.y,
            self.radius,
            color=aj.make_color_hsv(self.color_hue_angle, 1, 1),
        )
        ...
        aj.draw_text(
            (aj.room_width - aj.text_width(speed_text)) / 2,
            20,
            speed_text,
            color=aj.make_color_hsv(self.color_hue_angle, 1, 1),
        )
