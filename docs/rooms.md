...
If you have visual-only layers (for example an LDtk auto-layer export that should render but not
spawn gameplay objects), pass their names via `cosmetic_layers`:

```python
rooms = aj.load_ldtk_levels(
	project_dir / "rooms" / "world" / "simplified",
